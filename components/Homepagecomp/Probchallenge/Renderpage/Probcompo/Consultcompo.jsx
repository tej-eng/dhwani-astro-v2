"use client";

import { useState } from "react";
import useScrollZoom from "@/Hooks/scrollZoom";
import CustomButton from "@/components/Custom/CustomButton";
import Image from "next/image";
import Forminp from "@/components/Homepagecomp/Consultations/Concompo/Forminp";




export default function Consultcompo({ data }) {
    const [formDat, setformDat] = useState({
        name: "",
        dob: "",
        tob: "",
        pob: "",
        mail: "",
        num: "",
        gender: "",
        txt: "",
    });
    useScrollZoom(".head-wrap");

    const [showModal, setShowModal] = useState(false);

    return (
        <div className="consult-love flex flex-col items-center gap-2">
            <h1 className="relative head-wrap text-[#2f1254] text-md sm:text-2xl py-1 text-center font-semibold">
                • {data.name1} <strong>{data.name2}</strong> •
            </h1>

       
            <div className="md:grid md:grid-cols-5 flex flex-col justify-start md:gap-10 gap-5 text-black items-start px-5 py-3 shadow-lg border border-red-300 rounded-lg bg-gradient-to-r from-pink-100 to-yellow-100">
                <div className="head-wrap flex items-start justify-start w-[80%] place-self-center">
                    <Image
                        src={data.src}
                        alt="love image"
                        height={100}
                        width={100}
                        className="rounded-lg md:w-40 w-full h-50"
                    />
                </div>
                <div className="flex flex-col col-span-4">
                    <div className="head-wrap flex flex-col gap-2 items-center ">
                        <h5 className="md:text-xl text-base font-bold">{data.name}</h5>
                        <span className="text-base font-bold"> {data.price}</span>
                        <div className="flex flex-col gap-1 text-sm">
                            <h6 className="md:text-base text-sm font-semibold pt-2">
                                {data.que}{" "}
                            </h6>
                            <ul className="list-disc list-inside space-y-2 text-xs md:text-sm">
                                <li dangerouslySetInnerHTML={{ __html: data.li1 }}></li>
                                <li dangerouslySetInnerHTML={{ __html: data.li2 }}></li>
                                <li dangerouslySetInnerHTML={{ __html: data.li3 }}></li>
                            </ul>
                        </div>
                    </div>

                    <div className=" grid md:grid-cols-1 grid-cols-1 items-center justify-center gap-5">
                        <CustomButton 
                            aria-label="Book your Audio consultation"
                            variant={"gcircle"}
                            onClick={() => setShowModal(true)} 
                            className="flex items-center gap-2 next-cont justify-center w-[40%] mt-4 place-self-center bg-green-500 hover:bg-green-700 text-white font-semibold px-6 py-2 shadow"
                        >
                                    <svg width={20} height={20} viewBox="0 0 16 16" version="1.1" className="si-glyph si-glyph-call">


                              <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                                <path d="M14.031,11.852 C13.603,11.313 12.908,10.532 12.313,10.458 C11.951,10.413 11.535,10.713 11.125,10.996 C11.045,11.036 10.427,11.404 10.352,11.426 C9.956,11.539 9.111,11.572 8.6,11.106 C8.108,10.656 7.33,9.823 6.702,9.06 C6.102,8.274 5.473,7.329 5.151,6.749 C4.815,6.148 5.057,5.353 5.265,5.003 C5.303,4.94 5.763,4.467 5.866,4.357 L5.881,4.375 C6.262,4.055 6.661,3.73 6.706,3.378 C6.78,2.792 6.181,1.939 5.753,1.399 C5.325,0.858 4.662,-0.089 3.759,0.045 C3.419,0.095 3.126,0.214 2.837,0.385 L2.829,0.376 C2.823,0.38 2.795,0.402 2.781,0.413 C2.772,0.418 2.764,0.421 2.756,0.426 L2.759,0.43 C2.593,0.558 2.119,0.912 2.065,0.96 C1.479,1.481 0.597,2.708 1.279,4.915 C1.785,6.555 2.864,8.481 4.334,10.429 L4.326,10.436 C4.398,10.53 4.472,10.615 4.547,10.706 C4.617,10.799 4.686,10.891 4.758,10.983 L4.768,10.976 C6.328,12.855 7.964,14.357 9.457,15.243 C11.467,16.435 12.896,15.898 13.556,15.471 C13.618,15.43 14.09,15.063 14.25,14.942 L14.254,14.946 C14.26,14.94 14.264,14.932 14.272,14.926 C14.284,14.917 14.31,14.897 14.315,14.893 L14.309,14.885 C14.551,14.651 14.745,14.401 14.879,14.086 C15.23,13.257 14.459,12.393 14.031,11.852 L14.031,11.852 Z" fill="#fff" className="si-glyph-fill">

                                </path>
                              </g>
                            </svg>
                            <span className="text-xs md:text-sm text-center">
                                Book your Audio consultation
                            </span>
                        </CustomButton>
                    </div>
                </div>
            </div>

            {/* Modal */}
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-[#00000035] bg-opacity-50 z-50">
                    <Forminp formDat={formDat} setformDat={setformDat} onClose={() => setShowModal(false)}/>
                </div>
            )}
        </div>
    );
}
