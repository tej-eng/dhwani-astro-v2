...
//   });
    ...
    switch (active) {
      case "manglik":
        return (
          <>
            <Info
              label="Manglik Dosha Present"
              value={dosha.is_present ? "Yes" : "No"}
            />
            <Info label="Manglik Status" value={dosha.manglik_status} />
            <Info
              label="Percentage"
              value={`${dosha.percentage_manglik_present}%`}
            />
            <Info
      ...
      case "kal":
        return (
          <>
            <Info
              label="Kalsharp Dosha Present"
              value={dosha.present ? "Yes" : "No"}
            />
      ...
      case "sade":
        return (
          <>
            <Info
              label="Sade Sati Present"
              value={dosha.sadhesati_status ? "Yes" : "No"}
            />
            <Info
              label="Saturn Retrograde"
              value={dosha.is_saturn_retrograde ? "Yes" : "No"}
            />
            <Info label="Saturn Sign" value={dosha.saturn_sign} />
            <Info label="Moon Sign" value={dosha.moon_sign} />
            <Info
              label="Is Undergoing Sade Sati"
              value={dosha.is_undergoing_sadhesati ? "Yes" : "No"}
            />
            <Info
              label="What is Sadhesati?"
              value={dosha.what_is_sadhesati}
              block
            />
          </>
        );
  ...
  return (
    <div className="w-full sm:p-4 p-2">
      <h2 className="text-md sm:text-xl font-semibold text-center text-purple-700 mb-4">
        • Dosha in Kundli •
      </h2>
          ...
          return (
            <div
             ...
             >
              ...
              <span className="text-[10px] sm:text-xs bg-white text-black rounded-full px-2 py-1 mt-1">
                Present: <b>{isPresent ? "Yes" : "No"}</b>
              </span>
              <button
                onClick={() => setActive(d.id)}
                className={`mt-2 px-4 py-1 text-[10px] sm:text-xs font-semibold rounded-full ${active === d.id ? "bg-purple-600 text-white" : "bg-[#fff8] text-purple-600"}`}
              >
                View
              </button>
            </div>
      ...
      <div className="p-4 bg-purple-50 shadow-md rounded-lg border-2 border-violet-100">
        {renderDetails()}
        <CustomButton
          onClick={goToRem}
          className="mt-4 bg-purple-400 px-3 py-1 text-white hover:bg-purple-500 text-[12px] sm:text-sm rounded-full"
        >
