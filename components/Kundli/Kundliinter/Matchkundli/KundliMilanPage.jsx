...
const KundliMilanPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const field of requiredFields) {
      if (!boyData[field] || !girlData[field]) {
        setFormError(
          "Please fill all required fields for both Boy and Girl, including location.",
        );
        return;
      }
    }
    ...
    if (
      !/^\d{2}:\d{2}$/.test(boyData.birthTime) ||
      !/^\d{2}:\d{2}$/.test(girlData.birthTime)
    ) {
      setFormError(
        "Please enter valid birth times in HH:MM format for both Boy and Girl.",
      );
      return;
    }
    ...
    try {
      ...
      setFormError(
        "API error: " + (err?.data || err?.message || "Unknown error"),
      );
  ...
  const renderAstroBlock = (title, data, bgColor, txtColor) => (
    <div className={`bg-${bgColor}-100 p-4 rounded-lg shadow-md`}>
      <h5 className="font-semibold text-sm sm:text-md text-center mb-2">{title}</h5>
    ...
    <div className="min-h-screen flex flex-col items-center py-4 px-4">
      <h1 className="text-sm sm:text-lg text-black font-semibold">
        <span className=" text-sm sm:text-lg text-red-500">
          Free Match Making - Kundli Milan
        </span>
        Report
      </h1>
      ...
      {showForm ? (
        <div className="flex flex-col w-full gap-10">
          <form
            ...
            className="w-full space-y-6 pt-4 flex flex-col justify-center"
          >
            ...
            {formError && (
              <div className="text-center text-red-500 font-semibold text-sm py-2">
                {formError}
              </div>
            )}
            <button
          ...
          <div className="result-compat border border-gray-400 rounded-lg p-5 text-black flex flex-col gap-5 mt-5">
            <div className="head-wrap">
              <h5 className="text-center text-base text-black">
                ...
                to Check Possibilities of Marriage
              </h5>
              <span className="text-xs md:text-sm">
                <p>
                  Kundli milan or kundali matching is an important
                  consideration...
                </p>
                <p>
                  The online gun milan software on Astrotalk has been prepared
                  by the top astrologers...
                </p>
              </span>
            </div>
          </div>
            ...
            <div className="det-div w-full bg-white sm:p-6 p-4 rounded-lg shadow-xl">
              ...
              <div className="grid grid-cols-2 text-black md:grid-cols-2 gap-6 text-sm">
                <div className="bg-purple-200 text-xs sm:text-md p-4 rounded-lg flex flex-col gap-1">
                  <p>
                    <strong>Name:</strong> {boyData?.name}
                  </p>
                  <p>
                    <strong>Birth Date:</strong> {boyData?.dob}
                  </p>
                  <p>
                    <strong>Birth Time:</strong> {boyData?.birthTime}
                  </p>
                  <p>
                ...
                <div className="bg-pink-200 text-xs sm:text-md p-4 rounded-lg flex flex-col gap-1">
                  <p>
                    <strong>Name:</strong> {girlData?.name}
                  </p>
                  <p>
                    <strong>Birth Date:</strong> {girlData?.dob}
                  </p>
                  <p>
                  ...
                  {manglikData ? (
                    <>
                      {["male", "female"].map((gender) => (
                        <div key={gender} className="mb-6 text-xs sm:text-sm flex flex-col gap-1">
                          ...
                          <p className="flex gap-5">
                            <strong>
                              Manglik percentage After Cancellation :
                            </strong>{" "}
                            {
                              manglikData[gender]
