...
export default function Basic() {
  ...
  const [
    getBirthDetails,
    { data: birthData, isLoading: birthLoading, error: birthError },
  ] = useGetBirthDetailsMutation();
  const {
    data: astroData,
  ...
  if (birthError || astroError || bpError) {
    return <Message text="Failed to load reports." color="red" />;
  }
...
function DetailCard({ title, entries }) {
  return (
    <div className="w-full p-3 shadow-lg rounded-lg bg-white text-black">
      <h2 className="text-sm sm:text-xl font-bold text-center text-purple-700 mb-2">
        {title} 
      </h2>
      <div className="grid grid-cols-2 gap-4">
        {entries.map(([key, value]) => (
          ...
          >
            ...
            <span className="text-xs sm:text-sm ml-1">{value}</span>
          </div>
        ))}
      </div>
    </div>
