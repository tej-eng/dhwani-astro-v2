...
export default function Datecomp() {
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      dayDiv.className =
        "flex h-[38px] w-[38px] items-center justify-center rounded-[7px] border-[.5px] border-transparent text-dark hover:border-stroke hover:bg-gray-2 sm:h-[46px] sm:w-[47px] dark:text-black dark:hover:border-dark-3 dark:hover:bg-dark mb-2";
      ...
      dayDiv.addEventListener("click", () => {
        ...
        if (daysContainer) {
          daysContainer
            ...
            .forEach((d) => d.classList.remove("bg-primary", "text-black"));
          dayDiv.classList.add("bg-primary", "text-black", "dark:text-black");
        }
      });
  ...
  return (
    <section className="bg-white py-20 text-black dark:bg-dark">
        ...
        <div className="mx-auto w-full max-w-[510px]">
          <div className="relative mb-3">
            <input
              ...
              className="h-12 w-full appearance-none rounded-lg border border-stroke bg-white pl-12 pr-4 text-dark outline-none focus:border-primary dark:border-dark-3 dark:bg-dark-2 dark:text-black"
              value={selectedDate || ""}
              readOnly
            ...
            >
              <div className="flex items-center justify-between pb-4">
                <button aria-label="Previous Month"
                  id="prevMonth"
                  className="flex h-[38px] w-[38px] cursor-pointer items-center justify-center rounded-[7px] border-[.5px] border-stroke bg-gray-2 text-dark hover:border-primary hover:bg-primary hover:text-black sm:h-[46px] sm:w-[46px] dark:border-dark-3 dark:bg-dark dark:text-black"
                ...
                <span
                  id="currentMonth"
                  className="text-xl font-medium capitalize text-dark dark:text-black"
                >
              ...
              <div
                ...
                id="days-container"
                className="grid grid-cols-7 text-white text-center text-sm font-medium sm:text-lg"
              >
                {/* Days will be rendered here */}
              </div>

              <div className="flex items-center space-x-3 pt-4 sm:space-x-5">
                <button aria-label="Remove Date"
                  id="cancelBtn"
                  className="flex h-[50px] w-full items-center justify-center rounded-md bg-dark text-base font-medium text-black hover:bg-opacity-90"
                  onClick={handleCancel}
                >
                  Remove
                </button>
                <button aria-label="Apply Selected Date"
                  id="cancelBtn"
                  className="flex h-[50px] w-full items-center justify-center rounded-md bg-primary text-base font-medium text-black hover:bg-blue-dark"
                  onClick={handleApply}
                >
                  Done
                </button>
