...
export default function SingleButton({
  ...
  const chatredirect = () => {
    if (!user?.name) {
      toast.error(
        `${t?.pop?.pop1 || "You Are Offline. Please Connect to Login"}`,
      );
      return;
    }
    setAlert(true);
  ...
  return (
    <>
      {availability === 2 && (
        <button
          aria-label="Astrologer Busy"
          className="flex items-center gap-3 px-6  sm:px-8 py-2 text-white bg-yellow-500 rounded-full shadow md:py-3"
          onClick={astrologerbusy}
        >
          <div className="flex flex-col gap-1">
            ...
            <div className=" hidden items-center gap-3">
              ...
              <span className="font-semibold as-btn-free">{chat_price()}</span>
            </div>
          </div>
        </button>
      )}
      ...
      {availability == 1 && (
        <button
          ...
          className="flex items-center gap-3 px-6  sm:px-8 py-2 text-white bg-green-600 rounded-full shadow md:py-3"
      ...
      {availability == 0 && (
        <button
          aria-label="Astrologer Offline"
          className="flex items-center gap-3 px-6  sm:px-8 py-2 text-white bg-red-600 rounded-full shadow md:py-3"
          onClick={astrologeroffline}
        >
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-3">
              <svg width={18} height={18} viewBox="0 0 640 640">
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            <div className="hidden items-center gap-3">
              {" "}
              <span className="text-sm font-semibold line-through">
                ₹{astro_charge_call} / min
              </span>{" "}
              <span className="font-semibold as-btn-free">{call_price()}</span>
            </div>
          </div>
        </button>
      )}
